...
CHROME_DRIVER_PATH = path.join(path.split(path.abspath(__file__))[0], 'chromedriver.exe')
# ========================= Configuration Part =========================
...
def main():
    ...
    for news in news_list:
        if (news.link not in history and news.date == date_format
                and (not config["depart_select"]
                     or news.depart in config["depart_select"]) and
